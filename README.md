...
## 功能概览
- 成分股管理：支持 `xls/xlsx` 导入、手动增删改、一键等权分配。
- 股票代码处理：全链路按 6 位字符串处理（保留前导零，如 `002879`）。
- 调仓机制：支持多条调仓计划（每条可独立配置成分股和权重），并可叠加按月/按季度/自定义日期再平衡。
...
- Excel 导入会保留原始权重数值，回测时再做归一化，并使用高精度修正保证总和精确到 100%。
